...
@section('content')
    <div class="mb-6">
        ...
        <h1 class="text-3xl font-bold text-gray-900 dark:text-white">Pipeline d'Agents</h1>
        <p class="mt-2 text-sm text-gray-600 dark:text-gray-400">
            Visualització del processament del ticket {{ $ticket['id'] ?? 'N/A' }} pels agents d'IA
        </p>
    </div>
    ...
    @if(!$workflowResult)
        <div class="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-lg p-6">
            <p class="text-yellow-800 dark:text-yellow-200">
                No hi ha resultats del workflow disponibles. Torna al detall del ticket i processa'l amb els agents.
            </p>
        </div>
    @else
        <div class="space-y-6">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6">
                <h2 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">Resum del processament</h2>
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                        <dt class="text-xs text-gray-500 dark:text-gray-400">Estat final</dt>
                        <dd class="text-sm font-medium text-gray-900 dark:text-white mt-1">
                            <span class="px-2 py-1 rounded-full text-xs
                                ...
                                @endif">
                                {{ ucfirst(str_replace('_', ' ', $workflowResult['status'] ?? 'desconegut')) }}
                            </span>
                        </dd>
                    </div>
                    @if(isset($workflowResult['state']['type']))
                        <div>
                            <dt class="text-xs text-gray-500 dark:text-gray-400">Tipus detectat</dt>
                    ...
                    @if(isset($workflowResult['state']['area']))
                        <div>
                            <dt class="text-xs text-gray-500 dark:text-gray-400">Àrea</dt>
            ...
            <!-- Timeline d'agents -->
            <div class="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6">
                <h2 class="text-lg font-semibold text-gray-900 dark:text-white mb-6">Execució dels agents</h2>
                <div class="space-y-6">
                    @php
                    ...
                    @if(empty($trace))
                        <p class="text-sm text-gray-500 dark:text-gray-400">No hi ha traça disponible</p>
                    @endif
                </div>
            </div>
            ...
            <!-- Estat complet -->
            <div class="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6">
                <h2 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">Estat complet</h2>
            ...
            @if(isset($workflowResult['state']['shouldEscalate']) && $workflowResult['state']['shouldEscalate'])
                <div class="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-6">
                    <h3 class="text-base font-semibold text-blue-900 dark:text-blue-200 mb-2">Ticket escalat</h3>
                    <p class="text-sm text-blue-800 dark:text-blue-300">
                        {{ $workflowResult['state']['decisionReason'] ?? 'El ticket ha estat escalat per als agents.' }}
                    </p>
            ...
            @if(isset($workflowResult['state']['isSufficient']) && !$workflowResult['state']['isSufficient'])
                <div class="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-lg p-6">
                    <h3 class="text-base font-semibold text-yellow-900 dark:text-yellow-200 mb-2">Informació insuficient</h3>
                    <p class="text-sm text-yellow-800 dark:text-yellow-300 mb-2">
                        El ticket necessita més informació per ser processat:
                    </p>
